...
class SurfaceNets {
  ...
  static computeEdgeTable(cubeEdges: Uint32Array) {
    //Initialize the intersection table. This is a 2^(cube configuration) ->  2^(edge configuration)
    //  map There is one entry for each possible cube configuration, and the output is a 12-bit vector
    //  enumerating all edges crossing the 0-level.
  ...
  static readScalarValues(voxels, grid, dims, n, cols, mats) {
  ...
  static interpolateVertices(edgeMask, cubeEdges, grid, x, vertices) {
  ...
  static createFace(edgeMask, mask, buffer, R, m, x, faces) {
  ...
  static computeSurface(voxels) {
    ...
    for (x[2] = 0; x[2] < dims[2] - 1; ++x[2], n += dims[0], nbBuf ^= 1, R[2] = -R[2]) {
      ...
      //This is slightly obtuse because javascript does not have good support for packed data
      //structures, so we must use typed arrays :( The contents of the buffer will be the indices of
      //the vertices on the previous x/y slice of the volume
      var m = 1 + (dims[0] + 1) * (1 + nbBuf * (dims[1] + 1));

      for (x[1] = 0; x[1] < dims[1] - 1; ++x[1], ++n, m += 2) {
        for (x[0] = 0; x[0] < dims[0] - 1; ++x[0], ++n, ++m) {

          var mask = SurfaceNets.readScalarValues(voxels, grid, dims, n, cols, mats);
